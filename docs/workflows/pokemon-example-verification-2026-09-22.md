...
## Follow-up: slimmer CTA buttons

Applied the requested refinement consistently to collection/wishlist, card details, add-copy, and recovery actions. Android keeps the shared `DSButton`, lowers its visual minimum, and reduces footer padding from 12 to 8 dp and stacked-action spacing from 8 to 4 dp. Native screenshots measured the capsule changing from 105 to 95 pixels at 2.625× density (40 to approximately 36 dp); a tap above the visible capsule still opened the catalog, confirming the expanded native touch area. iOS now uses an example-local native action composition because the shared component reserves a 44-point label before system chrome. The compact variant retains regular system styling and body text, with a 30-point label minimum and a 44-point outer minimum. The native screenshot measured 132 pixels at 3× density (44 points), down from 58 points. Both grow naturally at larger text sizes. Shared template components remain unchanged.

- Android `scripts/verify --console=plain`: passed all 18 existing tests, Debug/Release lint, and both APK builds. A dedicated API 36 emulator exercised collection, catalog, details, add-copy, and successive saves through the smaller buttons; the compact CTA screenshot was captured. Emulator shut down and its task-owned AVD removed.
- iOS `scripts/verify --ui-only`: both interaction tests passed, including accessibilityXXXL dark Add/Save, with zero runtime warnings and successful simulator deletion. Evidence: `ios/.build/verification-20260922-212950-95ea95/`. Final Debug build-for-testing (including the new compact-action preview) and Release build both passed after the preview update; logs are `ios/.build/slim-cta-final-debug.log` and `ios/.build/slim-cta-final-release.log`.
- Updated screenshots: [iOS CTA](../../examples/pokemon-collection/docs/screenshots/ios-cta.png), [Android CTA](../../examples/pokemon-collection/docs/screenshots/android-cta.png). The README retains the initial populated-collection screenshots alongside links to the new CTA previews. Visual checks cover the standard and large-text states; physical-device and screen-reader limits above still apply.
