...
- For app/build changes, run `./tooling/scripts/verify ios` from the repository root. It compiles the app/package and test targets. Execute app and component tests separately with `./tooling/scripts/test ios`, or `test ios all` for all environments; the runner owns its disposable simulator. Native equivalents and artifacts are in `README.md`. Introduce meaningful behavior tests with product features.
